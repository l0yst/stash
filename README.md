<h1 align="center">SlateDust — AwesomeWM</h1>

<p align="center">
  <img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/main.png" width="800"/>
</p>

&nbsp;

> [!IMPORTANT]
> Check `pkgs.toml` for the full list of packages to install before setting up.

&nbsp;

## Setup

+ **OS**      — arch linux
+ **WM**      — [awesome wm](https://github.com/awesomeWM/awesome)
+ **Shell**   — zsh
+ **Term**    — [wezterm](https://github.com/wez/wezterm)
+ **Editor**  — [neovim](https://github.com/neovim/neovim) + [zed](https://github.com/zed-industries/zed)
+ **Browser** — [zen](https://zen-browser.app)
+ **Fetch**   — [fastfetch](https://github.com/fastfetch-cli/fastfetch)
+ **Prompt**  — [oh-my-posh](https://ohmyposh.dev)
+ **Comp**    — [picom](https://github.com/yshui/picom)
+ **Music**   — [amberol](https://gitlab.gnome.org/World/amberol)
+ **Icons**   — [Tela Circle](https://github.com/vinceliuice/Tela-circle-icon-theme)
+ **Cursor**  — [Bibata](https://github.com/ful1e5/Bibata_Cursor)
+ **Theme**   — [SlateDust](https://github.com/l0yst/slatedust)

&nbsp;

## Rofi

<table align="center"><tr>
<td align="center"><b>App Menu</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/apps.png" alt="app menu" width="400"></td>
<td align="center"><b>Apps as Root</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/appsasroot.png" alt="apps as root" width="400"></td>
</tr><tr>
<td align="center"><b>Power Menu</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/power-menu.png" alt="power menu" width="400"></td>
<td align="center"><b>Screenshoter</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/screenshoter.png" alt="screenshoter" width="400"></td>
</tr><tr>
<td align="center"><b>Emoji Picker</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/emoji-picker.png" alt="emoji picker" width="400"></td>
<td align="center"><b>Clipboard</b><br><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/clipboard.png" alt="clipboard" width="400"></td>
</tr></table>

&nbsp;

## Apps

<p align="center"><b>Spotify</b></p>
<p align="center"><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/spotify.png" alt="spotify"></p>

<p align="center"><b>GTK</b></p>
<p align="center"><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/file-manager.png" alt="gtk"></p>

<p align="center"><b>Terminal</b></p>
<p align="center"><img src="https://raw.githubusercontent.com/l0yst/stash/main/assets/github/slatedust/terminal.png" alt="terminal"></p>
&nbsp;

## Keybinds

| Keys | Action |
| --- | --- |
| `Super + /` | Show keybinds |
| `Super + Return` | Open terminal |
| `` Super + ` `` | Scratchpad terminal |
| `Super + a` | App launcher |
| `Super + x` | Power menu |
| `Super + c` | Kill window |
| `Super + Shift + space` | Toggle floating |
| `Super + Shift + r` | Restart awesome |
| `Print` | Screenshot |
| `Super + ,` | Clipboard manager |
| `Super + .` | Emoji picker |

&nbsp;

> [!NOTE]
> All configs are free to use — just credit me. Found a bug or want something added? [open an issue](https://github.com/l0yst/stash/issues).

&nbsp;

<p align="center">
  <sub>🪨 l0yst · slatedust · built on arch linux</sub>
</p>
